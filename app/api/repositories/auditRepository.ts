...
export class AuditRepository {
   ...
   */
  static addAuditTableRecord = async (params: AuditTableInsertParams): Promise<AuditLog> => {
    return withRetry(async () => {
      const { tableName, actionType, recordId, oldData, newData, actionBy } = params;
      try {
   ...
   */
  static updateAuditTableRecord = async (auditLog: AuditLog): Promise<AuditLog> => {
    return withRetry(async () => {
      try {
        await db`
          UPDATE audit_log
